...
const EditProfile = () => {
    ...
    const { errors , touched ,handleBlur, handleChange, handleSubmit} = useFormik({
        ...
        onSubmit : async (values) => {
            ...
            const finalData = { name, description, profile , email};
    ...
    <>
      ...
      <h1 className="mb-4 m-5 text-4xl tracking-tight font-extrabold text-center text-gray-900 dark:text-white">Profile</h1>
        ...
        <img className="w-24 h-24 mt-5 mb-5 rounded-full shadow-lg" src={image ? image : ((user?.profile?.url) ? user?.profile?.url : "https://images.unsplash.com/photo-1500648767791-00dcc994a43e?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=387&q=80")} alt="Bonnie image"/>
        <form method='POST' onSubmit={handleSubmit} className="space-y-4 md:space-y-6">
                    ...
                    <input className="block w-full text-sm text-gray-900 border border-gray-300 rounded-lg cursor-pointer bg-gray-50 dark:text-gray-400 focus:outline-none dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400" id="file_input" type="file" onChange={handleProfile} />
                    <div>
